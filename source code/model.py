...
from torch.nn.utils.rnn import pack_padded_sequdece, pad_packed_sequdece
...
class biLSTM(nn.Module):
    def __init__(self, vocab_size, emb_dim, hidden_size, out_size):
        ...
        self.bilstm = nn.LSTM(emb_dim, hidden_size,
    ...
    def forward(self, x, length):
        ...
        packed = pack_padded_sequdece(
            emb, length, batch_first=True, enforce_sorted=False)
        rnn_out, _ = self.bilstm(packed)
        rnn_out, _ = pad_packed_sequdece(rnn_out, batch_first=True)
    ...
    def validate(self, dev_word_lists, dev_tag_lists, word2id, tag2id):
        ...
        with torch.no_grad():
            ...
            for ind in range(0, len(dev_word_lists), utils.batch_size):
                ...
                tensorized_sents, lengths = utils.tensorize(
                    batch_sents, word2id, tag=False)
                tensorized_sents = tensorized_sents.to(utils.device)
                targets, lengths = utils.tensorize(
                    batch_tags, tag2id, tag=True)
            ...
            print("Saving...")
            with open('./model/loss{:.4f}.model'.format(val_loss), 'wb') as model_file:
