...
dl = data_loader()
test_data = dl.make_data_from_pkl('./dataset/train2.pkl')['data']
vocab = dl.make_data_from_pkl('vocab.pkl')
vocab_rev = dl.make_data_from_pkl('vocab_rev.pkl')
# with open('./saved_models/model_loss0.5736.model', 'rb') as model_file:
with open('./saved_models/model_loss0.8895.model', 'rb') as model_file:
    nn_model = pickle.load(model_file).to(utils.device)
...
#####################改######################
test_data = utils.sort_by_lengths_sub2(test_data)
print(len(test_data))
print(len(test_data[0]))
test_data2 = utils.sort_by_lengths_sub(test_data)[0]
print(len(test_data2))
print(len(test_data2[0]))
test_data_tensor, lengths = utils.tensorize(
...
# def get_key (dict, value):
#     return [k for k, v in dict.items() if v == value]
with open('./dataset/train.txt', encoding='utf8') as file:
    for line in file.readlines():
        if line != '\n':
            word = line[:-1].split('\t')
            # data2['Word'].append(word[0])
            data2['Tag'].append(word[-1])

with open('./train2.txt', 'w', encoding="utf-8") as result_file:
    n = 0
    for i in range(scores.shape[0]):
        data.append([])
        tag.append([])
        ...
        for j in range(lengths[i]):
            ...
            result[i].append(utils.tag_list_rev[torch.max(scores[i, j], 0)[1]])
            result_file.write('{}\t{}\t{}\n'.format(
                test_data[i][j], data2['Tag'][n], utils.tag_list_rev[torch.max(scores[i, j], 0)[1]]))
            n += 1
        result_file.write('\n')
